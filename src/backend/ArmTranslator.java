...
public class ArmTranslator {
    ...
    public ArmTranslator(ArrayList<IRCode> irList) {
        this.irList = irList;
    ...
    private void addPrints(IRCode code) {
        ...
        if (type.equals("str")) {
            ...
            if (printvar.isKindofsymbol()) {
                ...
                } else if (printvarsymbol.isGlobal() && printvarsymbol.getType() != Symbol.TYPE.F) {  //还要判断不是func返回值
    ...
    private void addGetint(IRCode code) {
        ...
        if (type.equals("var")) {     //读取到正常变量
            if (getintvar.isKindofsymbol()) {
                ...
                } else if (varsymbol.isGlobal() && varsymbol.getType() != Symbol.TYPE.F) {  //还要判断不是func返回值
            ...
            if (arrsymbol.isGlobal()) {    //全局取data段
                ...
                if (innerfunc) {  //array数组在函数内
                    if (curFunc.varnameIsFuncPara(arrayname)) {     //数组为 函数参数
                        ...
                        Symbol symbol = SymbolTable.lookupLocalTable(arrayname, Symbol.TYPE.I, arrsymbol.getScope());
...
//            pushinstrs.addInstr(new NewInstr("li $" + tmpregname + ", " + num));      //无pushoffset隐患
//            pushinstrs.addInstr(new NewInstr("sw $" + tmpregname + ", ", 0, "($sp)", "push"));
            ...
            if (var.isKindofsymbol()) {
                ...
                if (innerfunc && !varsymbol.isGlobal()) {    //函数内+symbol需要lw
                    ...
                    String globalvarname = varsymbol.getName();
                    pushinstrs.addInstr(new NewInstr("lw $" + tmpregname + ", Global_" + globalvarname));  //todo 也许不用加$zero
...
//                Instr last = new NewInstr("#push an symbolkind var end.");  //用一个#标签包装处理
...
//                pushinstrs.addInstr(new NewInstr("sw $" + varregname + ", ", 0, "($sp)", "push"));
//
//                //register.freeRegister(var);     ///不能放！因为实际还没存。。以下归还var
//                Instr last = new NewInstr("#push an nonsymbol(tmp)kind var end.");  //用一个#标签包装处理
//                last.hasRetReg = true;            //归还tmpregno
...
//                pushinstrs.addInstr(last);
            ...
            if (innerfunc && !arraysymbol.isGlobal()) {    //函数内+symbol需要lw
            ...
            } else if (arraysymbol.isGlobal() && arraysymbol.getType() != Symbol.TYPE.F) {     //全局数组
                ...
                if (var.getVar() != null) {     //处理如b[i]或b[1]等含偏移情况
                    ...
                    if (offsetType.equals("num")) {    //offset = 数字
                        ...
                        pushinstrs.addInstr(new NewInstr("la $" + tmpregname + ", Global_" + globalarrayname + "+" + arroffset));
                        ...
                        pushinstrs.addInstr(new NewInstr("la $" + tmpregname + ", Global_" + globalarrayname + "($" + tmpregname + ")"));

                        //以下处理： register.freeRegister(offset);
                        if (offset.getCurReg() != -1) {
                            //register.freeRegister(offset);  //统一释放存数组偏移量的reg.此处不能放

                            NewInstr last = new NewInstr("#push/la an hasoffset global array end.");  //用一个#标签包装处理
//                            last.hasRetReg = true;        //最后一个语句，附加一个归还offsetReg操作
//                            last.setFreeRegNumber(offset.getCurReg());  //todo getCurReg方法存疑
                            pushinstrs.addInstr(last);

                        } else {
                            pushinstrs.addInstr(new NewInstr("#push an hasoffset global array end."));  //用一个#标签包装处理);
                        }
                    }

                } else {
                    pushinstrs.addInstr(new NewInstr("la $" + tmpregname + ", Global_" + globalarrayname));
                }
...
//            pushinstrs.addInstr(new NewInstr("sw $" + tmpregname + ", ", 0, "($sp)", "push"));
//
//            Instr last = new NewInstr("#push an global array end.");  //用一个#标签包装处理
//            last.hasRetReg = true;            //归还tmpregno
//            last.setFreeRegNumber(tmpregno);  //释放的寄存器编号
//            pushinstrs.addInstr(last);
    ...
    private void addCall(IRCode code) {
        ...
        Symbol symbol = SymbolTable.lookupFullTable(funcname, Symbol.TYPE.F, SymbolTable.foreverGlobalScope);
        ...
        for (int i = pushwaitList.size() - paras; i < pushwaitList.size(); i++) {
            ...
            pushoffset -= 4;
            for (NewInstr pinstr : pushinstrs.getInstrList()) {
    ...
    private void addAssign2(IRCode code) {  //需要处理 左部 或 右部 是数组的情况
            ...
            if (symbol_oper1.isGlobal()) {    //全局取data段
                ...
                if (innerfunc) {  //array数组在函数内
                    if (curFunc.varnameIsFuncPara(arrayname)) {     //函数参数为数组时处理
                        ...
                        Symbol symbol = SymbolTable.lookupLocalTable(arrayname, Symbol.TYPE.I, symbol_oper1.getScope());
        ...
        if (typeDest.equals("array")) {
            ...
            if (symbol_dest.isGlobal()) {    //全局取data段
                ...
                if (innerfunc) {  //array数组在函数内
                    if (curFunc.varnameIsFuncPara(arrayname)) {     //数组为 函数参数
                        ...
                        Symbol symbol = SymbolTable.lookupLocalTable(arrayname, Symbol.TYPE.I, symbol_dest.getScope());
                        Assert.check(symbol, "MIPSTranslator / adAssign2() / dest / array / innerfunc");
            ...
            if (dest.isKindofsymbol()) {
                ...
                } else if (destsymbol.isGlobal() && destsymbol.getType() != Symbol.TYPE.F) {  //还要判断不是func返回值
    ...
    private void addFuncdefStmt(IRCode code) {
        ...
        switch (type) {
            case "funcDecl":
                ...
                Symbol symbol = SymbolTable.lookupFullTable(code.getName(), Symbol.TYPE.F, SymbolTable.foreverGlobalScope);
